...
import "./globals.css";
// Self-hosted Material Symbols icon font. Loaded from the npm package (served
// same-origin from /_next/static/media/, no Google Fonts request) so the
// `.material-symbols-outlined` class ships in our render-blocking stylesheet.
// That removes the first-load flash where icons showed as their raw ligature
// words (e.g. "settings help") and the buttons resized once the glyphs swapped
// in. The package CSS uses `font-display: block`, so glyphs stay blank (never
// the words) until the font loads.
    ...
    <html lang="en" className="dark">
      <head>
        ...
        */}
        ...
        {/* The Material Symbols icon font is self-hosted (imported at the top of
            this file) so there is no separate Google Fonts request that would
            delay the icons and resize buttons on first load. */}
