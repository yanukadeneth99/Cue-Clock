...
// ClockPicker default import removed: it was only rendered by the legacy
// (now-deleted) tail render branch. The modern home uses ClockRail instead.
...
// Only the TargetBlockType type survives: the TargetBlock component itself was
// rendered solely by the deleted legacy editing path (CueEditModal replaced it).
...
// The fullscreen TargetBlock font-sizing constants (FULLSCREEN_CLOCK_HEIGHT /
// _EXIT_BTN_HEIGHT / _MAX_FONT / _MIN_FONT / BLOCK_OVERHEAD) were removed: they
// only sized the legacy fullscreen TargetBlock list. OnAirView (the current
// fullscreen presenter) computes its own typography via urgencyFactor.
...
export default function HomeScreen() {
  // `height` is no longer destructured: it only fed the deleted fullscreen
  // TargetBlock font-sizing math. `width` still drives the mobile-web breakpoint.
  ...
  // Removed alongside the legacy render branch: safeBottom,
  // fullscreenAvailableHeight, blockCount, idealFontSize, countdownFontSize,
  // fullscreenNeedsScroll and notifUnavailableReason. They were consumed only by
  // the deleted fullscreen TargetBlock list; OnAirView and PrimaryCard own their
  // own sizing, and notification-unavailability is surfaced via the modal stack.
  ...
  // The legacy ClockPicker / TargetBlock / AlertModal render path was removed
  // here. Both branches above (`if (fullScreen)` -> OnAirView and
  // `if (!fullScreen)` -> the unified Header/PrimaryCard home) are exhaustive
  // for the boolean `fullScreen`, so this tail return was unreachable dead code.
  // Web and native now share the modern component tree, making the old
  // TargetBlock-based editing UI (and its exclusive handlers/constants) obsolete.
  return null;
}
