...
// TargetBlock's default export was only used by the now-removed legacy render
// path; only its TargetBlockType remains in use across the live state model.
...
export default function HomeScreen() {
  ...
  // The legacy render path (a third `return` rendering the old
  // ClockPicker / TargetBlock / per-block alert-modal layout) was removed here.
  // It was unreachable dead code: the `fullScreen` branch (OnAirView) and the
  // `!fullScreen` branch (the unified Header/PrimaryCard home) above already cover
  // every render case for both native and web, and web has no fullscreen entry
  // point that could fall through to this path. Carrying ~470 lines of dead JSX
  // (plus the now-unused ClockPicker import and TargetBlock default import) only
  // added bundle weight and maintenance drag to an app whose stated purpose is to
  // stay minimal and fast, so it has been deleted rather than kept for a later
  // cleanup commit.
}
