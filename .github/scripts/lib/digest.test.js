...
test("sriLankaDateLabel uses local date, not UTC date", () => {
  ...
  assert.strictEqual(
    sriLankaDateLabel(Date.parse("2026-07-19T21:00:00Z")),
    "2026-07-20",
  );
});
...
test("classify puts a human-review item in needsReview only, never both", () => {
  const out = classify(
    [
      item({
        labels: ["dependencies", "human-review"],
...
test("classify treats an unlabelled item as stuck no matter how new it is", () => {
  const out = classify(
    [item({ labels: [], createdAt: "2026-07-20T12:00:00Z" })],
    { nowMs: NOW, staleDays: STALE_DAYS },
  );
...
test("classify ignores a draft pull request even when it is old", () => {
  const out = classify(
    [item({ isDraft: true, createdAt: "2026-07-01T00:00:00Z" })],
    { nowMs: NOW, staleDays: STALE_DAYS },
  );
...
test("classify still reports a draft that carries human-review", () => {
  const out = classify([item({ isDraft: true, labels: ["human-review"] })], {
    nowMs: NOW,
...
test("classify still reports an in-progress item that carries human-review", () => {
  const out = classify(
    [
      item({
        labels: ["in-progress", "human-review"],
        createdAt: "2026-07-01T00:00:00Z",
      }),
    ],
    { nowMs: NOW, staleDays: STALE_DAYS },
...
test("classify leaves out a labelled item that is younger than the limit", () => {
  const out = classify([item({ createdAt: "2026-07-19T12:30:00Z" })], {
    nowMs: NOW,
    staleDays: STALE_DAYS,
  });
  assert.strictEqual(out.stuck.length, 0);
...
test("classify returns needsReview newest first and stuck oldest first", () => {
  ...
  assert.deepStrictEqual(
    out.needsReview.map((i) => i.number),
    [11, 10],
  );
  assert.deepStrictEqual(
    out.stuck.map((i) => i.number),
    [21, 20],
  );
});
...
test("trimComments drops the oldest comments first and reports the count", () => {
  const out = trimComments(
    [comment(1, 60), comment(2, 30), comment(3, 30)],
    100,
  );
  assert.strictEqual(out.omitted, 1);
  assert.deepStrictEqual(
    out.kept.map((c) => c.author),
    ["person2", "person3"],
  );
});
  ...
  const lines = formatStuckLines([
    {
      ...
      title: "first line\nsecond line",
      ageDays: 4,
      url: "https://x/issues/5",
    },
  ]);
  assert.strictEqual(lines.length, 1);
  assert.strictEqual(
    lines[0],
    "- #5 first line second line (4d) https://x/issues/5",
  );
});

test("buildDigestMessage leaves out the PR section entirely when there is no summary", () => {
  const msg = buildDigestMessage({
    summary: "",
    stuckLines: ["- #1 a (4d) u"],
    dateLabel: "2026-07-20",
    needsReviewOverflow: 0,
    stuckOverflow: 0,
  });
  assert.ok(!msg.includes("PRs:"));
  assert.ok(msg.includes("Issues:"));
});

test("buildDigestMessage leaves out the Issues section entirely when nothing is stuck", () => {
  const msg = buildDigestMessage({
    summary: "#7: needs a decision",
    stuckLines: [],
    dateLabel: "2026-07-20",
    needsReviewOverflow: 0,
    stuckOverflow: 0,
...
test("buildDigestMessage puts a blank line between two summary items", () => {
  const msg = buildDigestMessage({
    summary: "#7: first reason\n#8: second reason",
    stuckLines: [],
    dateLabel: "2026-07-20",
    needsReviewOverflow: 0,
    stuckOverflow: 0,
  });
  assert.ok(msg.includes("#7: first reason\n\n#8: second reason"));
});

test("buildDigestMessage puts a blank line between two stuck items", () => {
  const msg = buildDigestMessage({
    summary: "",
    stuckLines: ["- #1 a (4d) u", "- #2 b (5d) u"],
    dateLabel: "2026-07-20",
    needsReviewOverflow: 0,
    stuckOverflow: 0,
  });
  assert.ok(msg.includes("- #1 a (4d) u\n\n- #2 b (5d) u"));
});

test("buildDigestMessage does not double the gap when the summary already has blank lines", () => {
  const msg = buildDigestMessage({
    summary: "#7: first reason\n\n#8: second reason",
    stuckLines: [],
    dateLabel: "2026-07-20",
...
test("buildDigestMessage returns a message completely unchanged when well under the cap", () => {
  const msg = buildDigestMessage({
    ...
    stuckLines: ["- #1 title (5d) https://x/pull/1"],
    dateLabel: "2026-07-20",
    needsReviewOverflow: 0,
    stuckOverflow: 0,
  });
